...
def run_setup_wizard() -> None:
    ...
    from merco.core.config import PROVIDER_REGISTRY, ProviderInfo, MercoConfig
    ...
    # ── 步骤 1：选平台 ──
    provider = _pick_provider(PROVIDER_REGISTRY)
...
def _pick_provider(providers: dict) -> "ProviderInfo":
    """展示平台列表，让用户选择"""
    ...
    for i, p in enumerate(items, 1):
        table.add_row(str(i), p.name, p.default_model, p.description)
    # 自定义选项
...
def _ask_custom_provider() -> "ProviderInfo":
    """自定义平台：让用户输入必要信息"""
    from merco.core.config import ProviderInfo
    console.print("\n[bold]自定义平台[/bold]")
    ...
    return ProviderInfo(
        key=key, name=name, base_url="", key_env="",
        default_model="", models=[],
        key_help="", description="自定义平台",
    )


def _ask_api_key(provider: "ProviderInfo") -> str:
    """输入 API key"""
    console.print(f"\n[bold]第二步：配置 {provider.name} API Key[/bold]")
...
def _ask_model(provider: "ProviderInfo") -> str:
    """选择或输入模型名"""
    console.print(f"\n[bold]第三步：选择模型[/bold]")

    if provider.models:
        console.print(f"[dim]{provider.name} 已知模型:[/dim]")
...
def _ask_base_url(provider: "ProviderInfo") -> str:
    """输入或确认 base_url"""
...
def _confirm_and_save(provider: "ProviderInfo", api_key: str, model: str, base_url: str) -> None:
    """确认配置并写入 merco.json"""
    from merco.core.config import MercoConfig

    console.print(f"\n[bold]确认配置[/bold]")
    console.print(f"  平台:     {provider.name}")
    ...
    cfg.model.provider = provider.key
