...
| 日期 | 决策 | 原因 |
|------|------|------|
| 2026-08-05 | bash 工具 `stdin=DEVNULL` + `start_new_session=True` 隔离控制终端，不用 PTY（`1487e78`） | TUI 污染 merco 终端的根因是子进程共享控制终端 + 继承终端 stdin（经 /dev/tty 写转义序列、劫持输入），非 isatty。start_new_session 让子进程 setsid 后无控制终端（/dev/tty 打不开），DEVNULL 断输入劫持；stdout/stderr 仍走 PIPE 保输出干净、API 不变。PTY 是重武器：引入转义码污染输出、stdin EOF 死锁、stdout+stderr 合并。 |
| 2026-08-05 | MCP stdio 持久 session 复用，`AsyncExitStack` 管生命周期，失败重连一次（`119e462`） | 旧实现每次工具调用 fork 子进程 + MCP 握手（Python server 单次启动数百 ms）。`_connect_stdio`/`_connect_http` 用 AsyncExitStack 打开持久 ClientSession 返回 (session, tools, cleanup)，`_call_tool` 复用；call 抛异常（session 死）`_reconnect` 重连一次重试；disconnect/shutdown 调 cleanup 干净关闭。stdio 长连接是 MCP intended usage，旧 per-call fork 才是偏离。 |
| 2026-08-05 | MiniMax think-tag 用 find-last（插件层），核心保持 find-first（`1d587e2`） | reasoning 中字面闭标签被核心 find-first 误匹配截断。find-first/find-last 是对称权衡（字面标签在 reasoning vs content），无全局正解。MiniMax 语义只有一个 think 块、字面闭标签只在 reasoning，选 find-last。非流式 `_parse_response` 覆盖核心 find-first；流式 `chat_stream` 逐 chunk find-first（实时显示）+ 流末 `_reconcile` chunk 用 find-last 重切（字面/真闭标签常落不同 chunk）。核心保 find-first 不牺牲多块/字面-in-content。 |
| 2026-08-05 | `save_metadata` 加 `updated_at = now`，活动会话排在归档 clone 之前（`08c0b34`） | 每次启动归档+压缩循环的根因：clone_session（auto-fork）设 updated_at=now，save_metadata（持久化刷新后 checkpoint）不更新 updated_at -> 归档 clone 最新 -> resume 加载 clone（checkpoint 过期）-> 全量恢复+重新压缩+新 clone -> 循环。save_metadata 刷 updated_at 后活动会话（新鲜 checkpoint）领先归档 clone 被恢复。未用 `resume 排除 clone` 方案因其会让 bug 期间误存于 clone 的用户消息丢失。 |
...
| 2026-07-23 | Wave 3：删 6 个死文件（`97e70fc`, `ab20c07`） | `cli/tui.py`、`merco/tools/mcp_tools.py`（MCP 真身在 `merco/mcp/`，此为 stub 假实现）、`merco/gateway/{telegram,discord}.py`、`merco/scheduler/{delivery,jobs}.py`。无历史债务：删而非降级保留。 |
