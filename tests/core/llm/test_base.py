...
def test_model_provider_info_defaults():
    class FakeProvider(ModelProvider):
        name = "fake"
        async def chat(self, messages, tools=None, tool_choice=None): return {}
        def chat_stream(self, messages, tools=None, tool_choice=None):
            yield {}
    info = ModelProviderInfo(name="fake", provider_class=FakeProvider,
                             display_name="Fake")
...
def test_model_provider_info_strict_superset_no_dict_compat():
    info = ModelProviderInfo(name="x", provider_class=ModelProvider,
                             display_name="X")
    with pytest.raises(TypeError):
